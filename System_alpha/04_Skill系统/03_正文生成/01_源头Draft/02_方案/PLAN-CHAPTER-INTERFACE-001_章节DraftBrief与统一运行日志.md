...
1. 一份 confirmed 的可执行章节结构；
2. 一份 confirmed 章节 Draft Brief；
3. 一份对应本次生成的紧凑 Draft 运行日志。

其中“可执行章节结构”是正文生成的上游叙事输入，不属于正文模块新增的生成中间包。它可以来自独立章节大纲／细纲，但不能只是一句章节提要。进入 Draft Brief 前，至少应明确本章的 POV、核心功能、因果推进、关键信息释放与隐藏、章末状态或钩子，以及会改变正文方向的硬禁区。
...
- 优化 P0 等完整单元总纲时，先在总纲内逐章讨论并达到可拆分粒度，不因单章讨论完成就立即打断总纲工作；
- 单章在总纲内只有一句功能提要时，不足以直接进入正文生成；
- 完整单元总纲经作者确认后、某章进入独立正文对话前，再把该章已确认内容固化为独立的 confirmed 章节结构；
- 固化时只做结构提取、冲突校验和必要补缺，不擅自扩写新剧情；若发现会反向改变单元因果、人物弧线或信息阶梯的问题，先回到单元总纲修订；
- 独立章节结构确认后，才生成该章 Draft Brief；章节执行包仍非默认产物。

因此，P0C01 当前完成逐章讨论后，应继续服务于 P0 总纲的整体校验，不在总纲尚未确认时提前生成独立章纲。待 P0 总纲整体确认、P0C01 即将进入正文生产时，再固化其独立章节结构。
...
## 六、2026-07-24 总纲到章节结构边界

为避免“取消大型输出包”被误解为“正文可以没有章节结构”，本版补充确认：

1. 章节执行包继续退出默认链路；
2. 可执行章节结构是每章进入正文前的必需上游输入；
3. 单元总纲优化期间先完成全部章节及整体关系，不提前冻结单章文件；
4. 单元总纲确认后，再按实际正文生产顺序逐章固化独立章节结构并生成 Draft Brief。
