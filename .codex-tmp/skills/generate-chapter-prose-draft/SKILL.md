...
The draft must not depend on separate post-processing skills for de-AI cleanup, style consistency, or dialogue humor. Reusable prose-quality controls live in `references/director-prose-quality-rules.md`; chapter-specific expression differences come from the scene performance contract. Execute both during first-pass prose generation.
...
18. In `生成备注`, explicitly record whether only system sources were used, whether intent-control input was present, whether any non-system material pollution risk exists, and whether there are risks in character line win-rate, misread scale, main-hook focus, compressed pressure scenes, support-character emotional arc, word-budget drift, or scene-level reader experience.
